...
x=phrases.length
for(i=0;i<x;i++){
    if(phrases[i].length>phrases[y].length){

    
   longphrase=phrases[i];
   
   }
   document.write('la phrase la plus longue'+y)
}   
...
for (var i = 1; i < phrases.length; i++) {
    if (phrases[indexLePlusLong].length < phrases[i].length) {
        indexLePlusLong = i;
        console.log(indexLePlusLong);
    }
}

console.log(phrases[indexLePlusLong]);
